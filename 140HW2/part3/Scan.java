...
// ANSWER: putback is a flag that toggles between "eating comments mode" and "normal parsing mode".
// QUESTION 2: What are the values of putback and c just before the identifier
//             'hello' is returned from the input 'hello*45'?
// ANSWER: if putback is initailly false then putback = false and c = h 
            if( putback) {
                putback = false;
            }
            else {
            ...
            if ( myisalpha((char) c) ) {
                /* identifier. */
                String id = buildID(); //gets the entire identifier
...
// ANSWER: parses the symbols that aren't alphabets or digits. It translates the symbols used in E to the appprioate symbol in C. (is symbolo the right word?) 
                    case '/':
